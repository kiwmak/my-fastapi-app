...
try:
    # Thử import pymysql và patch MySQLdb
    import pymysql
    pymysql.install_as_MySQLdb()
    print("✅ Successfully patched MySQLdb with pymysql")
except ImportError:
    # Nếu không cài đặt pymysql, tạo một fake module để tránh lỗi
    print("⚠️ pymysql not installed, creating fake MySQLdb module")
    
    # Tạo một fake module cho MySQLdb
    class FakeMySQLdb:
        __version__ = "1.0.0"
        
        @staticmethod
        def connect(*args, **kwargs):
            raise ImportError("MySQLdb is not available. Please install pymysql.")
    
    sys.modules['MySQLdb'] = FakeMySQLdb()
    sys.modules['_mysql'] = None
...
class DatabaseManager:
    ...
    def ensure_table_exists(self):
        ...
        Sử dụng double quotes cho tên cột tiếng Việt.
        """
        if self.engine is None:
            logger.error("Không thể tạo bảng vì engine không tồn tại.")
            return
    ...
    def import_data(self, file_path):
        """
        Đọc file Excel và import vào PostgreSQL.
        Sử dụng ON CONFLICT để xử lý duplicate.
        """
        if self.engine is None:
            return {"success": False, "message": "Database connection is not available."}
        
        try:
            # --- Phần đọc và xử lý Pandas ---
            ...
            for _, row in df_result.iterrows():
                data_to_insert.append({
                    "kh": row.get("KHÁCH HÀNG", ""),
                    "dh": row.get("ĐƠN HÀNG", ""),
                    "mh": row.get("MÃ HÀNG", ""),
                    "kt": row.get("KÍCH THƯỚC", ""),
                    "mau": row.get("MÀU", ""),
                    "hl": row.get("HƯƠNG LIỆU", ""),
                    "bac": row.get("BẤC", ""),
                    "nt": row.get("NGÀY_TẠO", datetime.now())
                })

            # Insert dữ liệu
            with self.engine.begin() as conn:  # self.engine.begin() tự động commit/rollback
    ...
    def get_order_detail(self, order_no: str):
        ...
        try:
            with self.engine.connect() as conn:
                ...
                result = conn.execute(query, {"order_no": order_no}).fetchall()
...
class ExportManager:
    ...
    def get_cell_mapping(self, order_no: str, row_data: pd.Series) -> dict:
        ...
        mapping = {
            "C5": order_no,
            "C6": row_data.get("KHÁCH HÀNG", ""),
            "C7": row_data.get("HƯƠNG LIỆU", ""),
            "C8": row_data.get("MÀU", ""),
            "C9": row_data.get("BẤC", ""),
            "N5": row_data.get("MÃ HÀNG", ""),
            "N8": datetime.now().strftime("%Y-%m-%d")
        }

        kich_thuoc = str(row_data.get("KÍCH THƯỚC", ""))
        duong_kinh, chieu_cao = self.parse_kich_thuoc(kich_thuoc)

        mapping["N6"] = duong_kinh
        mapping["S6"] = chieu_cao
...
try:
    ...
    if db_manager.engine is None:
        logger.error("DatabaseManager được khởi tạo nhưng engine là None. Kiểm tra kết nối database.")
    else:
...
<body>
    <div class="container mt-4">
        <h1 class="text-center mb-4">🏭 Quản lý Test Đốt (PostgreSQL)</h1>
        ...
        <ul class="nav nav-tabs mb-4" id="mainTabs" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <a class="nav-link" href="/status" target="_blank">
                    <i class="fas fa-fire-alt me-2"></i>Hướng dẫn đánh giá
                </a>
            </li>
        </ul>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <small class="form-text text-muted">Tải lên logo mới</small>
                            </div>
    ...
    <script>
        ...
        async function importData() {
            ...
            try {
                ...
                const errorMessage = error.response ? error.response.data.detail : 'Lỗi kết nối';
            ...
            try {
                ...
                if (response.data.data.length > 0) {
                    ...
                    html += '</tbody></table></div>';
                     ...
                     html = '<div class="alert alert-warning mt-3">Không có chi tiết dữ liệu.</div>';
                }
                orderDetailDiv.innerHTML = html;
            } catch (error) {
                const errorMessage = error.response ? error.response.data.detail : 'Lỗi tải chi tiết';
            ...
            try {
                ...
                if (reports.length === 0) {
                    tbody.innerHTML = `
                        <tr>
                            <td colspan="5" class="text-center text-muted py-4">
                                <i class="fas fa-inbox me-2"></i>Chưa có báo cáo nào
                            </td>
                        </tr>
                    `;
                ...
                document.getElementById('reportsList').innerHTML = `
                    <tr>
                        <td colspan="5" class="text-center text-danger py-4">
                            <i class="fas fa-exclamation-triangle me-2"></i>Lỗi tải danh sách
                        </td>
                    </tr>
                `;
            }
        ...
        async function deleteReport(filename) {
            if (!confirm(`Xóa báo cáo "${filename}"?`)) return;
            try {
                const response = await axios.delete(`${API_BASE}/reports/${filename}`);
                showAlert('reportsCount', response.data.success, response.data.message);
                loadReports();
            } catch (error) {
        ...
        async function clearAllReports() {
            if (!confirm('Xóa TẤT CẢ báo cáo?')) return;
            try {
        ...
        async function uploadTemplate() {
            ...
            if (!file) {
                showAlert('templateResult', false, 'Chọn file template!');
                return;
            }
            ...
            try {
                ...
                const errorMessage = error.response ? error.response.data.detail : 'Lỗi tải lên';
        ...
        async function uploadLogo() {
            ...
            if (!file) {
                showAlert('templateResult', false, 'Chọn file logo!');
                return;
            }
...
app_fastapi = FastAPI(
    title="Quản lý Test Đốt",
    description="Ứng dụng quản lý test đốt nến",
    version="1.0.0"
)

# Mount static files
app_fastapi.mount("/templates", StaticFiles(directory=TEMPLATE_DIR), name="templates")
app_fastapi.mount("/exports", StaticFiles(directory=EXPORT_DIR), name="exports")

# Routes cho các file HTML (nếu có)
@app_fastapi.get("/nhietdo")
def nhiet_do():
    return FileResponse("nhiet_do_do_sap_web.html")

@app_fastapi.get("/muoi")
def muoi():
...
@app_fastapi.post("/api/import")
async def import_data_endpoint(file: UploadFile = File(...)):
    """API Import dữ liệu từ file Excel"""
    if not db_manager or db_manager.engine is None:
...
@app_fastapi.get("/api/orders")
async def get_orders_list():
    """API Lấy danh sách các đơn hàng"""
    if not db_manager or db_manager.engine is None:
        raise HTTPException(status_code=500, detail="Database not initialized")
    result = db_manager.get_orders_list()
    return result

@app_fastapi.get("/api/order/{order_no}")
...
@app_fastapi.post("/api/export-template/{order_no}")
async def export_report_endpoint(order_no: str):
    ...
    data_list = db_manager.get_order_detail(order_no)
    if not data_list:
        raise HTTPException(status_code=404, detail=f"Không tìm thấy dữ liệu: {order_no}")

    order_data_df = pd.DataFrame(data_list)
    
    if order_data_df.empty:
        raise HTTPException(status_code=404, detail=f"Không có dữ liệu để xuất: {order_no}")
...
@app_fastapi.get("/api/reports")
async def get_reports():
    """API Lấy danh sách các báo cáo đã tạo"""
    reports = export_manager.get_reports_list()
    return {"reports": reports, "count": len(reports)}

@app_fastapi.delete("/api/reports/{filename}")
async def delete_report_endpoint(filename: str):
    """API Xóa một báo cáo cụ thể"""
...
@app_fastapi.delete("/api/reports")
async def clear_all_reports_endpoint():
    """API Xóa tất cả các báo cáo"""
    count = 0
...
@app_fastapi.get("/api/download/{filename}")
async def download_report_endpoint(filename: str):
    """API Tải xuống báo cáo"""
    file_path = os.path.join(EXPORT_DIR, filename)
...
@app_fastapi.post("/api/upload-template")
async def upload_template_endpoint(file: UploadFile = File(...)):
    """API Tải lên file template MAU.xlsx mới"""
...
@app_fastapi.post("/api/upload-logo")
async def upload_logo_endpoint(file: UploadFile = File(...)):
    """API Tải lên file logo mới"""
    ext = file.filename.split('.')[-1].lower()
    if ext not in ["png", "jpg", "jpeg"]:
        raise HTTPException(status_code=400, detail="Logo phải là file PNG hoặc JPG")
...
async def health_check():
    return {"status": "healthy", "service": "test-dot-management"}

# Nếu chạy trực tiếp file này (cho mục đích development)
